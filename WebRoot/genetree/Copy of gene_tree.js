...
for(var i=0; i<rstlength; i++){
	...
	for(var k=0;k<length;k++){
		var gyY = topHeight+Math.floor(i/xNum)*height+allHeight/2-allGenHeight/2+yTemp;
		...
		if(j<yzlength-1)
		{
			if(j<circleNum){
				if(eachYZ[j].type == 'circle'){
					html += '<circle filter="'+eachYZ[j].filter+'" cx="'+circlex+'" cy="'+circley+'" r="'+r+'" onclick="clickYZ('+eachYZ[j].id+',\''+eachYZ[j].type+'\','+eachYZ[j].value+')" fill="'+eachYZ[j].color+'"/>';
		...
		else if(j==yzlength-1)
		{
			
			eachYZ[j].filter='type1';
			eachYZ[j].id='1';
...
function clickYZ(id,type,value){
	alert('可以在这里实现自己想要的逻辑，id:'+id+",type:"+type+",value:"+value);
}
...
function setFilterPan(tree){
	...
	var html = '';
	for(var i in data){
		html += '<div onclick="filter(\''+data[i].filter+'\')" class="filter-item">'+
					...
					'<div style="width:100%;float:left;">数据类型：'+data[i].filter+'</div>'+
				'</div>';
	}
